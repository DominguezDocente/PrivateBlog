...
namespace PrivateBlog.Web.Services
{
    ...
    {
        List<LogDTO> GetLogs(DateTime? date = null);
        List<LogDTO> GetLogsByPath(string logFilePath);
    }
    public class LocalLogSercive : ILogService
    {
        ...
        public List<LogDTO> GetLogsByPath(string logFilePath)
        {
        ...
        private void ParseAndAddLog(List<string> lines, List<LogDTO> targetList)
        {
            ...
            if (match.Success)
            {
                ...
                // Detectar método dentro del mensaje si existe
                string? method = null;
                var methodMatch = Regex.Match(messageStart, @"\(Method:\s*(?<method>[^\)]+)\)");
                if (methodMatch.Success)
                {
                    method = methodMatch.Groups["method"].Value.Trim();
                    // Opcional: quitar el método del mensaje principal si lo quieres separado
                    messageStart = Regex.Replace(messageStart, @"\(Method:\s*[^\)]+\)", "").Trim();
                }
