...
namespace PrivateBlog.Web.Services
{
    public class MailtrapService : IEmailService
    {
        ...
        public MailtrapService(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public async Task<Response<object>> SendAsync(SendEmailDTO dto)
        {
            try
            {
                ...
                {
                    throw new ApplicationException("No está registrada el ApiKey de Mailtrap en el archivo de configuración");
                }
                ...
                BaseApiService apiService = new BaseApiService();

                Response<object> response = await apiService.PostAsync<object>(url, request, headers);
